...
export default async function handler(req: any, res: any) {
  try {
    const all = await dbContemp('users');
    ...
    for await (let exist of all) {
      ...
      if (!exist.backup_picture) update.backup_picture = exist.picture

      if(Object.keys(update).length > 0) {
        await dbContemp('users')
          .update(update)
          .where('id', exist.id);
      }

      if (exist.picture){
        const url = exist.picture;
        ...
        await dbContemp('users')
          .update('picture', `${baseUrl}api/pictures/${fileName}`)
          .where('id', exist.id);
      }
